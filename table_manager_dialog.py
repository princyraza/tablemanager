...
# FORM_CLASS, _ = uic.loadUiType(os.path.join(os.path.dirname(__file__), 'table_manager_dialog_base.ui'))
MAIN_DIALOG, _ = uic.loadUiType(os.path.join(os.path.dirname(__file__), 'tableManagerUi.ui'))
RENAME_DIALOG, _ = uic.loadUiType(os.path.join(os.path.dirname(__file__), 'tableManagerUiRename.ui'))
CLONE_DIALOG, _ = uic.loadUiType(os.path.join(os.path.dirname(__file__), 'tableManagerUiClone.ui'))
INSERT_DIALOG, _ = uic.loadUiType(os.path.join(os.path.dirname(__file__), 'tableManagerUiInsert.ui'))

# class TableManagerDialog(QtWidgets.QDialog, FORM_CLASS):
...
class DialogRename(QtWidgets.QDialog, RENAME_DIALOG):
  def __init__(self, iface, fields, selection, parent=None):
  ...
  def accept(self):
    ...
    for i in self.fields.values():
      if self.newName().upper() == i.name().upper() and i != self.fields[self.selection]:
        QtWidgets.QMessageBox.warning(self,self.tr('Rename field'),self.tr('There is another field with the same name.\nPlease use a different one.'))
        return
    if not self.newName():
      QtWidgets.QMessageBox.warning(self,self.tr('Rename field'),self.tr('The new name cannot be empty.'))
...
class DialogClone(QtWidgets.QDialog, CLONE_DIALOG):
  ...
  def accept(self):
    if not self.result()[1]:
      QtWidgets.QMessageBox.warning(self,self.tr('Clone field'),self.tr('The new name cannot be empty.'))
      return
    if self.result()[1] == self.fields[self.selection].name():
        QtWidgets.QMessageBox.warning(self,self.tr('Clone field'),self.tr('The new field\'s name must be different than the source\'s!'))
        return
    for i in self.fields.values():
      if self.result()[1].upper() == i.name().upper():
        QtWidgets.QMessageBox.warning(self,self.tr('Clone field'),self.tr('There is another field with the same name.\nPlease use a different one.'))
        return
    QtWidgets.QDialog.accept(self)
...
class DialogInsert(QtWidgets.QDialog, INSERT_DIALOG):
  def __init__(self, iface, fields, selection):
    QtWidgets.QDialog.__init__(self)
    self.iface = iface
    self.setupUi(self)
  ...
  def accept(self):
    if not self.result()[0]:
      QtWidgets.QMessageBox.warning(self,self.tr('Insert new field'),self.tr('The new name cannot be empty.'))
      return
    for i in self.fields.values():
      if self.result()[0].upper() == i.name().upper():
        QtWidgets.QMessageBox.warning(self,self.tr('Insert new field'),self.tr('There is another field with the same name.\nPlease use a different one.'))
        return
    QtWidgets.QDialog.accept(self)
...
class TableManagerDialog(QtWidgets.QDialog, MAIN_DIALOG):
  ...
  def doDelete(self): # Called when appropriate button was pressed
    ...
    if QtWidgets.QMessageBox.warning(self, self.tr('Delete field'), warning , QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No) == QtWidgets.QMessageBox.No:
        return
  ...
  def doSave(self): # Called when appropriate button was pressed
    ...
    if not QFile(srcPath+'.dbf').rename(srcPath+'.dbf~'):
      QtWidgets.QMessageBox.warning(self, self.tr('Table Manager'), self.tr('Failed backing up the old table to {0}.dbf~\nThe layer won\'t be changed, please use the Save As button.').format(srcPath))
    ...
    # copy the .dbf from the temp directory to the target location
      ...
      if not QFile(srcPath+'.dbf~').rename(srcPath+'.dbf'):
        QtWidgets.QMessageBox.warning(self, self.tr('Table Manager'), self.tr('WARNING! I can neither save the new {0}.dbf file\nnor restore it from the {0}.dbf~ backup.\nPlease check it manually!').format(srcName))
        QgsVectorFileWriter.deleteShapeFile(tmpDir+'/'+srcName+'.shp')
        return
      # dbf~ file restored :
      QtWidgets.QMessageBox.warning(self, self.tr('Table Manager'), self.tr("Failed saving the changes to {0}.dbf\nThe layer will not be changed, please use the Save As button.").format(srcPath))
      '''
      QgsVectorFileWriter.deleteShapeFile(tmpDir+'/'+srcName+'.shp')
      return
      we don't return now because layer has to be reloaded
    ...
    if not newLayer.isValid():
      QtWidgets.QMessageBox.warning(self, self.tr('Table Manager'), self.tr("WARNING! The changes seem to be commited, but I can't reload the layer!\nPlease check it out!\nThe old table is backed up as {0}.dbf~.").format(srcName))
      return
    # copy the style (it's possible only if the clasyfying field was not renamed!)
    if QtWidgets.QMessageBox.question(self, self.tr('Saving successful'),self.tr('Saving successful. The old table has been backed up as {0}.dbf~.\nDo you wish to keep the layer style?\n\nNote that if the style depends on an attribute you\'ve renamed, all features on the layer will become invisible. In that case please adjust the style manually.').format(srcName), QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No) == QtWidgets.QMessageBox.Yes:
      '''#newLayer.copySymbologySettings(self.layer)'''
      resp = newLayer.loadNamedStyle( tmpDir+'/'+srcName+'.qml' )
      if not resp[1]:
        QtWidgets.QMessageBox.warning(self, self.tr('Table Manager'), self.tr("This layer will be reloaded without its previous style (loading style failed)"))
    QFile.remove( tmpDir+'/'+srcName+'.qml' )
    # set encoding
  ...
  def doSaveAs(self): # Called when appropriate button was pressed
    ...
    if filePath == self.provider.dataSourceUri().split('|')[0]:
      if QtWidgets.QMessageBox.question(self, self.tr('Table Manager'), self.tr('You are attemping to save the changes to the original file. Are you sure you want to do this? If yes, the original layer will be removed from the legend.'), QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No) == QtWidgets.QMessageBox.Yes:
    ...
    if layer.isValid():
      ...
      QtWidgets.QMessageBox.warning(self, self.tr('Table Manager'), self.tr("WARNING! The new layer seems to be created, but is invalid.\nIt won't be loaded."))
  ...
  def writeToFile(self, filePath, encoding, driverName = 'ESRI Shapefile'): # write data to a shapefile
    if QFile(filePath).exists():
      try:
        ...
        QtWidgets.QMessageBox.warning(self, self.tr('Table Manager'), self.tr('Cannot overwrite an existing shapefile.\nPlease remove it manually.'))
        return 1
    ...
    if writer.hasError():
      QtWidgets.QMessageBox.warning(self, self.tr('Table Manager'), self.tr("Error creating file. The errror message was:") + "<br/>" + writer.errorMessage() )
      self.progressBar.reset()
      return 2
    geom = QgsGeometry()
    n = 0
  ...
  def doDone(self): # Called when 'Done' button pressed
    ...
    if not (self.isUnsaved and QtWidgets.QMessageBox.question(self, self.tr('Table Manager'), question, QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No) == QtWidgets.QMessageBox.No):
      self.close()
